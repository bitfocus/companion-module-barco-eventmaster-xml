	...
	initActions() {
		this.setActionDefinitions({
			...
			audioTone: {
				...
				callback: async (event) => {
					const cmd = `<System id="0" GUID="a0afbd8052e0-003b54" OPID="-1"><DestMgr id="0"><ScreenDestCol id="0"><ScreenDest id=${event.options.destination}><AudioTestTone>${event.options.onOff}</AudioTestTone></ScreenDest></ScreenDestCol></DestMgr></System>`
					this.log(
						'info',
						`Sending audio tone command: ${event.options.onOff} to destination: ${event.options.destination}`,
					)
					this.sendCommand(cmd)
				},
			},
